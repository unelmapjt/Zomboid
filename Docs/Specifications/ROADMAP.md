...
- [-] **2.4 Cinematic Sequence**
  ...
  - [ ] **残作業**: 3ページ目で足元に B42 血液 Fluid を生成（吐血演出）。
  - [ ] **残作業**: 5ページ目のボタンラベルを `getText("UI_NE_Confirm")` に変更（現状は実装済みだが翻訳キー未定義）。
...
- [x] **3.2 Key Item Assignment**
  ...
  - [ ] **残作業**: 未回収リマインダー: プレイヤーが 10 タイル離れたら Dr.Hiro への誘導 Say を発動。
...
- [ ] **4.3 Indoor Erosion System**
  室内の侵食度 `InternalErosion` の動的追跡 (設計書 §4.7)。
...
- [ ] **6.2 Erosion Retardant**
  侵食遅延剤（30分 ΔMutation ゼロ固定）の実装 (設計書 §8.1.2)。
  - 服用後 30 分間 `DeltaMutation` を 0 に固定する。
  - HUD に残り時間タイマーを表示する (§8.2)。
  - アイテム ID: `NOX_EVO_B42.NE_ErosionRetardant`。
...
- [ ] **7.4 Burster (バースター)**
  ...
  - [ ] 検証: バースター撃破後にガスが発生すること。

- [ ] **7.5 Stalker & Tracker**
  - ストーカー: 無音接近・背後強襲。
  - トラッカー: 過去 10 分のパン屑追跡 + 血汚れ感知強化。
  - [ ] 検証: トラッカーが移動経路を正確に追従すること。
...
- [ ] **9.3 Indoor Visual Sync**
  室内侵食ビジュアルとデトックス可視化 (設計書 §4.9.3)。
  - `InternalErosion` 上昇に伴い緑色の霞を室内に滲ませる。
  - 除菌範囲内では侵食演出を一時リセット。
...
- [ ] **11.1 Outbreak Surge**
  日数別の強化波動 (設計書 §10.3)。
  - Day 10/20/30/40 に増大する強襲をスケジュール。
...
- [ ] **12.1 LoreManager — Brain Announcements**
  `NE_LoreManager.lua` による脳内アナウンス (設計書 §14.4)。
  - 特定日数・変異閾値到達時に画面上部に「本能テキスト」を表示。
  - [ ] 検証: 設計書 §9.1 の各日程テキストが表示されること。

- [ ] **12.2 Lore Tapes**
  変異種撃破時のカセットテープドロップと再生 (設計書 §17.2)。
  - 特定変異種の撃破時にテープをドロップ (確率設定)。
  - [ ] 検証: トラッカー撃破時にテープがドロップする確率動作を確認。

---

## Phase 13: EndGame (設計書 §7, §11.1)

- [ ] **13.1 Day 50 Airstrike**
  50 日目の空爆ソフトデッドライン (設計書 §7.3)。
  - `NE_EndGame.lua` で Phase 4 移行時に HEF 連携イベントを発火。
  - Day 45〜 から本能テキストによる誘導。
  - [ ] 検証: Day 50 で空爆イベントが発火すること。

- [ ] **13.2 B17 Terminal Trigger**
  B17 最深部端末への接近でエンディング UI を起動 (設計書 §7.2)。
  - 指定座標に近接 + `B17_AccessCardKey` 所持チェック。
  - カード未所持の場合は警告 Say のみ。
  - [ ] 検証: カードなしで端末に近づくと警告が出ること。

- [ ] **13.3 3 Endings**
  PURIFICATION / SYMBIOSIS / DESTRUCTION の 3 分岐 (設計書 §7.4)。
  - 各エンドで音楽・演出・字幕が異なる。
  - [ ] 検証: 3 つのエンディングが全て到達可能であること。
...
- [ ] **14.3 Challenge Mode**
  `Challenge_NOX_EVOLVED.lua` による超加速侵食チャレンジの完成 (設計書 §13)。
...
- [-] **X.1 Debug Control Panel**
  ...
  - [ ] **残作業**: EP・フェーズ・フィルター寿命の直接操作。
  - [ ] **残作業**: 症状トグル / マイルストーン強制発火。
