...
- **必須参照**: 新規の実装、特にカメラ、視覚エフェクト、エンティティ生成を行う際は、必ず `memory-bank/failures.md` を確認し、禁止されているパターンを踏んでいないかチェックすること。
- **失敗の記録**: Java 例外やクラッシュを引き起こした API は、解決後直ちに `memory-bank/failures.md` へ追記し、再発を防止すること。
