# [FROZEN] NOX_VERIFIED_API — Legacy Whitelist

> ⚠️ **このファイルは凍結（FROZEN）されています。新規追記禁止。**
> 唯一の真実源（Ground Truth）は `.cursor/registry/api_whitelist.md` に移行しました。
> API の確認・追記は `.cursor/registry/` 以下のファイルを使用してください。
> 本ファイルは旧参照の互換性のために保持されますが、内容は更新されません。
